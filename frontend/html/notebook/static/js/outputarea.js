...
var IPython = (function (IPython) {
    ...
    var OutputArea = function (selector, prompt_area) {
        ...
        this.plotsName = new Array(); // The generates plots file for the next output print.
        this.dointIt = false;
    };
    ...
    OutputArea.prototype.handle_output = function (msg_type, content) {
        ...
        } else if (msg_type === "pyout") {	// Only show python output when console is set to Python
    ...
    OutputArea.prototype.append_output = function (json, dynamic) {
        ...
        if (json.output_type === 'pyout' && IPython.notebook.currentCode === 'code') {
        ...
        if(!this.doingIt && json.output_type != 'pyerr' && IPython.notebook.currentCode==='codeR' && this.plotsName.length>0) {
        	this.doingIt = true;
        	this.append_svg_for_R();
        	this.doingIt = false;
        };
    ...
    OutputArea.prototype.append_svg_for_R = function() {
    	
    	for(var i=0; i<this.plotsName.length; i++) {
    		var elem =  this.create_output_area();
        	var flexBox = $("<div/>").addClass("box-flex1 output_subarea output_svg");
        	var img = $("<img/>").attr('src',"/files/temp/"+this.plotsName[i]);
        	this._dblclick_to_reset_size(img);
        	flexBox.append(img);
        	elem.append(flexBox);
        	this.element.append(elem);
    	}
    	this.plotsName = new Array();
