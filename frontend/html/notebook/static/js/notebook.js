...
var IPython = (function (IPython) {
    ...
    var Notebook = function (selector) {
        ...
        this.bind_events();
        this.currentCode = 'code';		// The Python console
        this.executedR = false;		// Flag to indicate that R sentences have been executed at some point.
    ...
    Notebook.prototype.create_elements = function () {
        ...
        var that = this;
        var end_space = $('<div/>').addClass('end_space').height("50px");
        end_space.dblclick(function (e) {
            if (that.read_only) return;
            var ncells = that.ncells();
            that.insert_cell_below(that.currentCode,ncells-1);
        });
    ...
    Notebook.prototype.bind_events = function () {
        ...
        $([IPython.events]).on('set_next_input.Notebook', function (event, data) {
            ...
            var new_cell = that.insert_cell_below(that.currentCode,index);
    ...
    Notebook.prototype.insert_cell_below = function (type, index) {
        ...
        if (this.ncells() === 0 || this.is_valid_cell_index(index)) {
            if (type === 'code') {
                ...
                cell = new IPython.CodeCellR(this.kernel);
                cell.set_input_prompt();
    ...
    Notebook.prototype.to_codeR = function (index) {
        var i = this.index_or_selected(index);
        this.currentCode = 'codeR';
        if (this.is_valid_cell_index(i)) {
            var source_element = this.get_cell_element(i);
            var source_cell = source_element.data("cell");
            if (!(source_cell instanceof IPython.CodeCellR)) {
                target_cell = this.insert_cell_below('codeR',i);
                var text = source_cell.get_text();
                if (text === source_cell.placeholder) {
                    text = '';
                }
    ...
    Notebook.prototype.all_to_codeR = function (index) {
        this.currentCode = 'codeR';
        var iSel = this.index_or_selected(index);
    	var cells = this.get_cell_elements();
    	var len = cells.length;
    	for (var i=0; i<len; i++) {
    		var source_element = this.get_cell_element(i);
    		var source_cell = source_element.data("cell");
            if ((source_cell instanceof IPython.CodeCell) || (iSel === i)) {
                target_cell = this.insert_cell_below('codeR',i);
                var text = source_cell.get_text();
                if (text === source_cell.placeholder) {
                    text = '';
    ...
    Notebook.prototype.all_to_code = function (index) {
        this.currentCode = 'code';
        var iSel = this.index_or_selected(index);
    	var cells = this.get_cell_elements();
    	var len = cells.length;
    	for (var i=0; i<len; i++) {
    		var source_element = this.get_cell_element(i);
    		var source_cell = source_element.data("cell");
    		if ((source_cell instanceof IPython.CodeCellR) || (iSel === i)) {
                target_cell = this.insert_cell_below('code',i);
                var text = source_cell.get_text();
                if (text === source_cell.placeholder) {
                    text = '';
    ...
    Notebook.prototype.split_cell = function () {
        ...
        if (cell.is_splittable()) {
            ...
            if (cell instanceof IPython.CodeCell) {
                ...
                cell.set_text(texta);
                var new_cell = this.insert_cell_below('codeR');
                new_cell.set_text(textb);
    ...
    Notebook.prototype.execute_selected_cell = function (options) {
        ...
        default_options = {terminal: false, add_new: true, isFile: false};
        ...
        if (cell instanceof IPython.CodeCell) {
        	...
        	cell.execute(default_options.isFile);
        	code = 'codeR';			// For R console
        } else if (cell instanceof IPython.HTMLCell) {
            cell.render();
        }
        if (default_options.terminal) {
            //cell.select_all();	by iMathResearch S.L.
        } else {
    ...
    Notebook.prototype.load_notebook_success = function (data, status, xhr) {
        this.fromJSON(data);
        if (this.ncells() === 0) {
            this.insert_cell_below(this.currentCode);
        };
        this.dirty = false;
        this.select(0);
        this.scroll_to_top();
